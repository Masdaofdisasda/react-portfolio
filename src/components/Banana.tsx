...
function Banana({ index, z, speed }) {
  const ref = useRef<THREE.LOD<THREE.Object3DEventMap>>()
  ...
  const { nodes, materials } = useGLTF('../images/banana-v2-transformed.glb')
  ...
  useFrame((state: RootState, dt: number) => {
    ...
    if (dt < 0.1 && ref.current != null) ref.current.position.set(index === 0 ? 0 : data.x * width, (data.y += dt * speed), -z)
    // Rotate the object around
    if (ref.current != null) ref.current.rotation.set((data.rX += dt / data.spin), Math.sin(index * 1000 + state.clock.elapsedTime / 10) * Math.PI, (data.rZ += dt / data.spin))
    ...
    >
      ...
      <EffectComposer enableNormalPass={true} multisampling={0}>
        ...
        <SSAO
          ...
          radius={0.2}
          bias={0.05}
          normalDepthBuffer={null} // No downsampling here
          resolutionScale={0.5} // Render at half resolution for performance
        />
